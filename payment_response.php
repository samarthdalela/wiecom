...
try {
    ...
    <head>
        ...
        <title>UPWIECON 2026 - Payment Result</title>
        <style>
            * {
                margin: 0;
                padding: 0;
        ...
        <div class="result-container">
            ...
            <div class="details-section">
                ...
                <div class="detail-grid">
                    ...
                    <div class="detail-item">
                        ...
                        <span
                            class="detail-value"><?php echo htmlspecialchars($responseData['transaction_id'] ?: 'N/A'); ?></span>
                    </div>
                    <?php if (!empty($responseData['gateway_reference'])): ?>
                        <div class="detail-item">
                            <span class="detail-label">Gateway Reference:</span>
                            <span
                                class="detail-value"><?php echo htmlspecialchars($responseData['gateway_reference']); ?></span>
                        </div>
                    <?php endif; ?>
                    <div class="detail-item">
                    ...
                    <?php if (!empty($responseData['error_message']) && $responseData['status'] !== 'SUCCESS'): ?>
                        <div class="detail-item full-width">
                            ...
                            <span class="detail-value"
                                style="color: #721c24;"><?php echo htmlspecialchars($responseData['error_message']); ?></span>
                        </div>
                    <?php endif; ?>
                </div>
            ...
            <div style="margin-top: 30px;">
                <?php if ($responseData['status'] === 'SUCCESS'): ?>
                    ...
                    <a href="mailto:support@upwiecon2026.org?subject=UPWIECON2026 Registration Confirmation&body=Registration ID: <?php echo $registrationDetails['iRegId']; ?>%0AOrder ID: <?php echo $responseData['order_id']; ?>%0ATransaction ID: <?php echo $responseData['transaction_id']; ?>"
                        class="btn btn-secondary">Contact Support</a>
                <?php else: ?>
                    <a href="registrationform.php" class="btn btn-primary">Try Again</a>
                    <a href="mailto:support@upwiecon2026.org?subject=UPWIECON2026 Payment Issue&body=Order ID: <?php echo $responseData['order_id']; ?>%0ATransaction ID: <?php echo $responseData['transaction_id']; ?>%0AAmount: ₹<?php echo $responseData['amount']; ?>%0AStatus: <?php echo $responseData['status']; ?>"
                        class="btn btn-secondary">Contact Support</a>
                <?php endif; ?>
            </div>
            ...
            <?php if (isset($_GET['debug']) && $_GET['debug'] === '1'): ?>
                <div
                    style="background: #f8f9fa; border: 1px solid #dee2e6; border-radius: 5px; padding: 15px; margin: 20px 0; text-align: left;">
                    <h4>Debug Information:</h4>
                    <details>
                        <summary>Raw Response</summary>
                        <pre
                            style="font-size: 11px; overflow-x: auto;"><?php echo htmlspecialchars(json_encode($rawResponse, JSON_PRETTY_PRINT)); ?></pre>
                    </details>
                    <details>
                        <summary>Processed Response</summary>
                        <pre
                            style="font-size: 11px; overflow-x: auto;"><?php echo htmlspecialchars(json_encode($responseData, JSON_PRETTY_PRINT)); ?></pre>
                    </details>
                </div>
            <?php endif; ?>
        </div>
    ...
    <head>
        <meta charset="UTF-8">
        <meta name="viewport" content="width=device-width, initial-scale=1.0">
        <title>UPWIECON 2026 - Payment Error</title>
        <style>
            body {
                font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif;
                background: linear-gradient(135deg, rgba(70, 12, 82, 0.49) 0%, rgba(70, 12, 82, 0.99) 100%);
    ...
    <body>
        <div class="error-container">
            ...
            <a href="mailto:support@upwiecon2026.org?subject=UPWIECON2026 Payment Processing Error&body=Reference ID: <?php echo date('YmdHis') . '_' . substr(md5($e->getMessage()), 0, 8); ?>%0AError: <?php echo urlencode($e->getMessage()); ?>"
                class="btn btn-secondary">Contact Support</a>
        </div>
    </body>
...
function sendProductionEmail($registrationDetails, $responseData)
{
    ...
    $subject = "UPWIECON 2026 - Payment " . $responseData['status'];
    ...
    if ($responseData['status'] === 'SUCCESS') {
        $message .= "Your registration for UPWIECON 2026 is confirmed!\n\n";
        $message .= "Conference Details:\n";
        $message .= "- Event: IEEE Uttar Pradesh Women in Engineering Conference 2026\n";
        ...
        $message .= "We look forward to your participation in UPWIECON 2026!\n\n";
    } else {
    ...
    $message .= "Best regards,\nUPWIECON 2026 Organizing Committee\n";
    $message .= "Email: support@upwiecon2026.org\n";
    $message .= "Website: https://upwiecon2026.org";

    $headers = "From: UPWIECON 2026 <noreply@upwiecon2026.org>\r\n";
    $headers .= "Reply-To: support@upwiecon2026.org\r\n";
    $headers .= "Content-Type: text/plain; charset=UTF-8\r\n";
    $headers .= "X-Mailer: UPWIECON2026 Production System\r\n";
...
{
    $to = $registrationDetails['sEmail'];
    $subject = "UPWIECON 2026 - Payment " . $responseData['status'] . " [Ref: " . $responseData['order_id'] . "]";
    ...
    $message .= "<h1 style='margin: 0; font-size: 24px;'>UPWIECON 2026</h1>";
    ...
    if ($responseData['status'] === 'SUCCESS') {
        ...
        $message .= "<p style='margin: 0;'>Your registration for UPWIECON 2026 has been confirmed successfully.</p>";
        $message .= "</div>";
    } else {
    ...
    $message .= "<p style='margin: 0; color: #666;'>Best regards,<br><strong>UPWIECON 2026 Organizing Committee</strong></p>";
    $message .= "<p style='font-size: 12px; color: #999; margin: 10px 0 0;'>For support, contact: <a href='mailto:support@upwiecon2026.org' style='color: #460C52;'>support@upwiecon2026.org</a></p>";
    $message .= "</div>";
...
{
    $adminEmail = "admin@upwiecon2026.org"; // Change to your admin email
    $subject = "UPWIECON 2026 - Payment Notification [" . $responseData['status'] . "]";
    ...
    $headers = "From: UPWIECON System <system@upwiecon2026.org>\r\n";
    $headers .= "Content-Type: text/plain; charset=UTF-8\r\n";
